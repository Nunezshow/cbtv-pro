...
index 0b58a050773870f5d48d6c6f301e3fffd33fb4a3..216b0dba0d919054618003863b91a040a0a08e05 100644
...
+            val lastChannel by remember(rememberLast) {
+                if (rememberLast) settings.getLastChannel() else flowOf(null)
+            }.collectAsState(initial = null)
+
             var channels by remember { mutableStateOf<List<Channel>>(emptyList()) }
             val scope = rememberCoroutineScope()
+            var navigatedToLast by remember { mutableStateOf(false) }
...
+                    LaunchedEffect(channels, lastChannel, rememberLast) {
+                        if (!navigatedToLast && rememberLast && lastChannel != null && channels.isNotEmpty()) {
+                            nav.navigate("${Routes.PLAYER}/${Uri.encode(lastChannel!!)}")
+                            navigatedToLast = true
+                        }
+                    }
+
                     NavHost(
...
--- a/app/src/main/java/com/cbuildz/tvpro/MainActivity.kt
+++ b/app/src/main/java/com/cbuildz/tvpro/MainActivity.kt
@@ -123,44 +136,47 @@ class MainActivity : ComponentActivity() {
                                             favoritesUrls + channel.url
                                         }
...
+                                onClearLastChannel = {
+                                    scope.launch { settings.clearLastChannel() }
+                                },
                                 onBack = { nav.popBackStack() }
                             )
                         }
